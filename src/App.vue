...
export default {
  ...
  methods: {
    ...
    log(message) {
      // create a log function to write to file
      const userId = this.TMA.userId;
      fs.appendFile(this.logFile, `${new Date().toISOString()} - ${userId} - ${message}\n`, (err) => {
        if (err) {
          console.error(err);
        }
      });
    ...
    processQRCode(data) {
      ...
      this.log(`QR code scanned: ${data.data} by user ${this.TMA.userId}`);
